import { books } from '../assets/Data'
import { Album } from 'lucide-react'
import dayjs from 'dayjs'

export default function BorrowedDue() {
  // Filter only books that are due today
  const dueToday = books.filter(book => book.status === "Due Today")

  const currentDate = dayjs().format("MMMM D, YYYY")

  return (
    <div className="mt-4 flex-center gap-4 w-full p-4 px-8">
      <div className="w-full  rounded-lg shadow border border-gray-200 overflow-hidden">
        {/* Header */}
        <div className="p-3 border-b border-gray-200 flex items-center justify-between">
          <h2 className="font-bold  text-lg text-gray-700 flex items-center gap-2">
            <Album className="text-blue-600" size={20} /> Books Due Today
          </h2>
          <span className="text-sm text-gray-500">{currentDate}</span>
        </div>

        {/* Table */}
        <table className="w-full text-sm text-left text-gray-700">
          <thead className="bg-blue-100 text-blue-700 uppercase text-xs font-semibold">
            <tr>
              <th className="px-4 py-3">#</th>
              <th className="px-4 py-3">Book Details</th>
              <th className="px-4 py-3">Borrower Name</th>
              <th className="px-4 py-3">Transaction ID</th>
              <th className="px-4 py-3">Borrowed Date</th>
              <th className="px-4 py-3">Due Date</th>
              <th className="px-4 py-3">Availability</th>
              <th className="px-4 py-3 text-center">Status</th>
            </tr>
          </thead>

          <tbody>
            {dueToday.length > 0 ? (
              dueToday.map((book, index) => (
                <tr
                  key={index}
                  className="border-b border-gray-100 hover:bg-blue-50 transition"
                >
                  <td className="px-4 py-3 font-medium text-gray-600">
                    {index + 1}
                  </td>

                  {/* Book Details */}
                  <td className="px-4 py-3 flex items-center gap-2">
                    <img
                      src={book.coverImage}
                      alt="Book cover"
                      className="w-10 h-14 object-cover rounded"
                    />
                    <div className="flex flex-col">
                      <span className="font-semibold">{book.title}</span>
                      <span className="text-xs text-gray-500 italic">{book.author}</span>
                      <span className="text-xs text-gray-400">Year: {book.year}</span>
                    </div>
                  </td>

                  {/* Borrower */}
                  <td className="px-4 py-3">{book.borrowerName || "—"}</td>

                  {/* Transaction ID */}
                  <td className="px-4 py-3">{book.transactionID || "—"}</td>

                  {/* Borrowed Date */}
                  <td className="px-4 py-3">{book.borrowedDate || "—"}</td>

                  {/* Due Date */}
                  <td className="px-4 py-3 text-blue-600 font-semibold">
                    {book.dueDate || "—"}
                  </td>

                  {/* Availability */}
                  <td className="px-4 py-3">
                    <span
                      className={`${
                        book.availability === "Available"
                          ? "text-green-600"
                          : "text-red-600"
                      } font-medium`}
                    >
                      {book.availability}
                    </span>
                  </td>

                  {/* Status */}
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${
                        book.status === "Due Today"
                          ? "bg-red-100 text-red-600"
                          : "bg-green-100 text-green-600"
                      }`}
                    >
                      {book.status}
                    </span>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td
                  colSpan="8"
                  className="text-center text-gray-500 py-6 italic"
                >
                  No books due today.
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>
    </div>
  )
}

